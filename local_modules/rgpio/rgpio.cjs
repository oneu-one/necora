...
// コマンド送信（拡張データあり形式）
async function _lg_command_ext_nolock(sl_s, cmd, p3, extents, Q = 0, L = 0, H = 0) {
    try {
...
// sbc() の名前でエクスポートすると pigpio や rgpio の python ぽい使い方ができる！
// Usage: const sbc = await rgpio.sbc();
async function create_sbc(host = process.env.LG_ADDR || 'localhost', port = process.env.LG_PORT || 8889, show_errors = true) {
...
class _sbc {
    ...
    async init(host = process.env.LG_ADDR || 'localhost', port = process.env.LG_PORT || 8889, show_errors = true) {
        this.connected = true;
        this.sl_s = null;
    ...
    async gpiochip_open(gpiochip) {
        ...
        let handle = u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GO, 4, [extBuf], 0, 1, 0));
        if (handle >= 0) {
    ...
    async gpiochip_close(handle) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GC, 4, [extBuf], 0, 1, 0));
    }
    ...
    async gpio_get_chip_info(handle) {
        ...
        bytes = u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GIC, 4, [extBuf], 0, 1, 0));

        let lines = 0, name = "", label = "";
        if (bytes > 0) {
    ...
    async gpio_claim_output(handle, gpio, level = 0, lFlags = 0) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GSOX, 16, [extBuf], 0, 4, 0));
    }
    ...
    async gpio_claim_input(handle, gpio, lFlags = 0) {
        ...
        extBuf.writeUInt32LE(lFlags, 4);
        extBuf.writeUInt32LE(gpio, 8);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GSIX, 12, [extBuf], 0, 3, 0));
    }
    ...
    async gpio_read(handle, gpio) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GR, 8, [extBuf], 0, 2, 0));
    }
    ...
    async gpio_write(handle, gpio, level) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GW, 12, [extBuf], 0, 3, 0));
    }
    ...
    async gpio_free(handle, gpio) {
        ...
        extBuf.writeUInt32LE(handle & 0xffff, 0);
        extBuf.writeUInt32LE(gpio, 4);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_GSF, 8, [extBuf], 0, 2, 0));
    }

    // PWM
    async tx_pwm(handle, gpio, pwm_frequency, pwm_duty_cycle, pulse_offset = 0, pulse_cycles = 0) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_PX, 24, [extBuf], 0, 6, 0));
    }
    ...
    async i2c_open(bus, addr, flags = 0) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CO, 12, [extBuf], 0, 3, 0));
    }
    ...
    async i2c_close(handle) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CC, 4, [extBuf], 0, 1, 0));
    }
    ...
    async i2c_read_byte(handle) {
        const extBuf = Buffer.alloc(4);
        extBuf.writeUInt32LE(handle, 0);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CRS, 4, [extBuf], 0, 1, 0));
    }
    ...
    async i2c_write_byte_data(handle, reg, byte_val) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CWB, 12, [extBuf], 0, 3, 0));
    }
    ...
    async i2c_read_byte_data(handle, reg) {
        ...
        extBuf.writeUInt32LE(handle, 0);
        extBuf.writeUInt32LE(reg, 4);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CRB, 8, [extBuf], 0, 2, 0));
    }
    ...
    async i2c_write_word_data(handle, reg, word_val) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CWW, 12, [extBuf], 0, 3, 0));
    }
    ...
    async i2c_read_word_data(handle, reg) {
        ...
        extBuf.writeUInt32LE(handle, 0);
        extBuf.writeUInt32LE(reg, 4);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CRW, 8, [extBuf], 0, 2, 0));
    }
    ...
    async i2c_write_i2c_block_data(handle, reg, data) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CWI, 8 + dataBuf.length, [extBuf, dataBuf], 0, 2, 0));
    }
    ...
    async i2c_read_i2c_block_data(handle, reg, count) {
        ...
        const bytes = u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CRI, 12, [extBuf], 0, 3, 0));
        if (bytes > 0) {
            const rdata = await this._rxbuf(bytes);
            return _u2i_list([bytes, rdata]);
        } else {
    ...
    async i2c_write_device(handle, data) {
        ...
        extBuf.writeUInt32LE(handle, 0);
        const dataBuf = Buffer.from(data);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_I2CWD, 4 + dataBuf.length, [extBuf, dataBuf], 0, 1, 0));
    }
    ...
    async serial_open(tty, baud, ser_flags = 0) {
        ...
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_SERO, 8 + ttyBuf.length, [extBuf, ttyBuf], 0, 2, 0));
    }
    ...
    async serial_close(handle) {
        const extBuf = Buffer.alloc(4);
        extBuf.writeUInt32LE(handle, 0);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_SERC, 4, [extBuf], 0, 1, 0));
    }
    ...
    async serial_write(handle, data) {
        ...
        extBuf.writeUInt32LE(handle, 0);
        const dataBuf = Buffer.from(data);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_SERW, 4 + dataBuf.length, [extBuf, dataBuf], 0, 1, 0));
    }
    ...
    async serial_read(handle, count = 1000) {
        ...
        extBuf.writeUInt32LE(handle, 0);
        extBuf.writeUInt32LE(count, 4);
        const bytes = u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_SERR, 8, [extBuf], 0, 2, 0));
        if (bytes > 0) {
            const rdata = await this._rxbuf(bytes);
            return _u2i_list([bytes, rdata]);
        } else {
    ...
    async serial_data_available(handle) {
        const extBuf = Buffer.alloc(4);
        extBuf.writeUInt32LE(handle, 0);
        return _u2i(await _lg_command_ext_nolock(this.sl_s, _CMD_SERDA, 4, [extBuf], 0, 1, 0));
    }
