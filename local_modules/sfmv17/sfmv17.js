...
class SFMV17 {
  ...
  sendCmd = async (cmdType, p1, p2, p3) => {
    while (await rg.serial_data_available(this.ser_hand))
      rg.serial_read(this.ser_hand);
    let cmdBuffer = [0xf5, cmdType, p1, p2, p3, 0, 0, 0xf5];
    cmdBuffer[6] = this.getCheckSum(cmdBuffer);
    await rg.serial_write(this.ser_hand, cmdBuffer);
  };
  getAck = async () => {
    let ackBuffer = Buffer.alloc(0); //Buffer.from([]);
    let timer = this.SFM_SERIAL_TIMEOUT;
    while (timer--) {
      if ((await rg.serial_data_available(this.ser_hand)) > 0) {
        ackBuffer = Buffer.concat([
          ackBuffer,
          await rg.serial_read(this.ser_hand, 0),
        ]);
      } else if (ackBuffer.length >= 8) {
        // 1/100秒待ってデバイス側にデータが残っていないか再チェック
        // await delay(10);
        await rg.lgu_sleep(0.01);
        timer -= 10;
        if ((await rg.serial_data_available(this.ser_hand)) > 0) continue;
        // もうデータは残っていないらしい
  ...
  init = async (serial_port, baud = 115200) => {
    this.ser_hand = await rg.serial_open(serial_port, baud);
  ...
  stop = async () => {
    if (this.ser_hand >= 0) {
      await rg.serial_close(this.ser_hand);
      this.ser_hand = -1;
    }
  };
}
